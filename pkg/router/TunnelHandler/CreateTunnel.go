...
func HandleCreateTunnel(c *gin.Context, db *gorm.DB) {
	...
	if proxy.ProxyType != "tcp" && proxy.ProxyType != "udp" && proxy.ProxyType != "http" && proxy.ProxyType != "https" {
		c.JSON(400, gin.H{"error": "proxy_type is not valid"})
		return
	}
	...
	if !IsvalidIP(proxy.LocalIP) {
		c.JSON(400, gin.H{"error": "local_ip is not valid"})
		return
	}
	if !IsvalidPort(proxy.LocalPort) {
		c.JSON(400, gin.H{"error": "local_port is not valid"})
		return
	}
	if proxy.ProxyType == "http" || proxy.ProxyType == "https" {
		if !IsValidDomain(proxy.Domain) {
			c.JSON(400, gin.H{"error": "domain is not valid"})
			return
		}
	} else {
		if !IsvalidPort(proxy.RemotePort) {
			c.JSON(400, gin.H{"error": "remote_port is not valid"})
			return
		}
	}
	...
	if proxyCount > 0 {
		c.JSON(400, gin.H{"error": "proxy name already exists"})
		return
	}
	...
	if userResult.Error != nil {
		c.JSON(500, gin.H{"error": userResult.Error.Error()})
		return
	}
	// 获取用户的 group 值
	var group Group
	...
	if groupResult.Error != nil {
		c.JSON(500, gin.H{"error": groupResult.Error.Error()})
		return
	}
	...
	if len(proxies) >= limit.Proxies {
		c.JSON(400, gin.H{"error": "proxy count limit"})
		return
	}
	// 检查 node 是否存在
	var nodeCount int64
	...
	if nodeCount == 0 {
		c.JSON(400, gin.H{"error": "node not exists"})
		return
	}
	...
	if proxy.ProxyType == "tcp" || proxy.ProxyType == "udp" {
		...
		if proxyCount2 > 0 {
			c.JSON(400, gin.H{"error": "remote port already exists"})
			return
		}
	} else {
